...
impl<'info> ResolveBet<'info> {
    pub fn verify_ed25519_signature(&mut self, sig: &[u8]) -> Result<()> {
        let instruction =
            load_instruction_at_checked(0, &self.instruction_sysvar.to_account_info())
        ...
        require_eq!(instruction.accounts.len(), 0, DiceError::Ed25519Accounts);
        require!(instruction.data.len() != 0 , DiceError::Ed25519DataLength);

        let signatures = Ed25519InstructionSignatures::unpack(&instruction.data)
        ...
        require_keys_eq!(
            signature.public_key.ok_or(DiceError::Ed25519Header)?,
            self.house.key(),
    ...
    pub fn resolve_bet(&mut self, sig: &[u8], bumps: &ResolveBetBumps) -> Result<()> {
        ...
        let mut hash_id_len: [u8; 16] = [0; 16];
        hash_id_len.copy_from_slice(&hash[0..16]);
        let lower = u128::from_le_bytes(hash_id_len);
        hash_id_len.copy_from_slice(&hash[16..32]);
        let upper = u128::from_le_bytes(hash_id_len);

        let roll = lower.wrapping_add(upper).wrapping_rem(100) as u8 + 1;

        if self.bet.roll > roll {
            let pay_out = { self.bet.amount as u128 }
            ...
            transfer(cpi_ctx, pay_out as u64)?;
        }
        Ok(())
    }
}
